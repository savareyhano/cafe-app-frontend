          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-beige/10">
                <th className="p-3 text-[10px] font-mono tracking-widest text-[#dfc6b3] uppercase">Order ID / Time</th>
                <th 
                ...
                >
                  <div className="flex items-center gap-1">Table <ArrowUpDown size={12} /></div>
                </th>
                <th className="p-3 text-[10px] font-mono tracking-widest text-[#dfc6b3] uppercase">Items</th>
                <th 
                  className="p-3 text-[10px] font-mono tracking-widest text-[#dfc6b3] uppercase cursor-pointer hover:text-wood"
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-beige/50 font-mono text-sm">Loading orders...</td>
                </tr>
              ) : data?.data?.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-beige/50 font-mono text-sm">No orders found.</td>
                </tr>
              ) : (
                data?.data?.map((order: any) => (
                  <tr key={order.id} className="border-b border-beige/5 hover:bg-forest/30 transition-colors align-top">
                    <td className="p-3">
                      <p className="font-mono text-xs text-beige/60" title={order.id}>
                        {order.id.slice(0, 8)}...
                      </p>
                    ...
                    <td className="p-3">
                      <p className="font-serif text-white text-base">{order.customerName}</p>
                      <p className="text-xs text-beige/50 font-mono">{order.customerPhoneNumber}</p>
                    </td>
                    <td className="p-3">
                      ...
                      <div className="space-y-1">
                        {order.orderItems?.map((item: any) => (
                          <div key={item.id} className="text-xs">
                            <span className="font-mono text-wood">{item.quantity}x</span>{' '}
                            <span className="text-beige/80">{item.menuItems.name}</span>
                            {item.note && (
                              <p className="text-[10px] text-beige/40 italic ml-4">Note: {item.note}</p>
                            )}
                          </div>
                        ))}
                      </div>
                    ...
                    <td className="p-3">
                      <DropdownMenu>
                        <DropdownMenuTrigger className="flex items-center gap-2 px-3 py-1.5 rounded-lg border font-mono text-[10px] uppercase tracking-widest focus:outline-none focus:ring-1 focus:ring-wood transition-colors bg-earth-dark/50 hover:bg-forest outline-none border-beige/10">
                          <span className={`px-2 py-0.5 rounded border ${statusColors[order.status] || 'bg-beige/10'}`}>
                            {order.status}
                          </span>
                          <ChevronDown size={14} className="text-beige/50" />
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="bg-earth-dark border-beige/10 text-beige">
                          {['pending', 'ready', 'served', 'paid'].map((status) => (
                            <DropdownMenuItem 
                              key={status}
